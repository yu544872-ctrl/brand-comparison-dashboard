...
def gen(title, sub, l1, l2, l3, l4, l5, ml, pl, bt, lang):
    ...
    def render_analysis_table(month):
        ...
        for row in rows:
            ...
            row_class = ' class="special-blogger"' if (row['authorId'] or '').casefold() == 'ecoflow3303' else ''
            html += '<tr' + row_class + '><td>' + escape(type_label) + '</td><td>' + escape(row['author'] or '-') + '</td><td class="blogger-id">' + escape(row['authorId'] or '-') + '</td>'
